...
def main():
    ...
    for input_ids, input_mask, segment_ids, label_ids in eval_dataloader:
        ...
        with torch.no_grad():
            ...
            encoded_layers,logits = model(input_ids, segment_ids, input_mask)
        
        encoded_layers=torch.stack(encoded_layers)
        encoded_layers=encoded_layers.cpu().numpy().tolist()
    ...
    with open(os.path.join(args.output_dir, "encoded_layers_{0}.json".format(index_cnt)), "w") as fout:
        fout.write(json.dumps(encoded_layers, indent=2) + "\n")
